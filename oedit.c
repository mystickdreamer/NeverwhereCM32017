...
void oedit_disp_weapon_menu( DESCRIPTOR_DATA *d )
{
    ...
    {
	sprintf( buf, "&g%2d&w) %-20.20s ", counter, weapon_skill[counter] );
	if ( ++col % 2 == 0 )
	    strcat( buf, "\n\r" );
	send_to_char_color( buf, ch );
    }
...
// Menu of the bane flags on weapons
void oedit_disp_bane_menu( DESCRIPTOR_DATA *d )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *ch = d->character;
    int counter, col = 0;
    ...
    {
	sprintf( buf, "&g%2d&w) %-20.20s ", counter, weapon_bane[counter] );
	if ( ++col % 2 == 0 )
	    strcat( buf, "\n\r" );
	send_to_char_color( buf, ch );
    }
...
// Branding of a weapon ex: flaming, vorpal, etc
void oedit_disp_brand_menu( DESCRIPTOR_DATA *d )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *ch = d->character;
    int counter, col = 0;
    ...
    {
	sprintf( buf, "&g%2d&w) %-20.20s ", counter, weapon_brand[counter] );
	if ( ++col % 2 == 0 )
	    strcat( buf, "\n\r" );
	send_to_char_color( buf, ch );
    }
    send_to_char( "\n\rEnter Brand type: ", ch );
...
{
    send_to_char( "Enter the name of the spell - -1 to skip: ", d->character );
}
...
void oedit_disp_val3_menu( DESCRIPTOR_DATA *d )
{
    ...
    case ITEM_STAFF:
 	...
 	break;
    case ITEM_WEAPON:
	send_to_char("Damage Bonus : ", d->character);
	break;
    case ITEM_CONTAINER:
...
void oedit_disp_val4_menu( DESCRIPTOR_DATA *d )
{
	...
	break;
	case ITEM_WEAPON:
	//send_to_char("Attack Type : ", d->character);
...
void oedit_disp_wear_menu( DESCRIPTOR_DATA *d )
{
	...
	sprintf( buf, "&g%2d&w) %-20.20s ", counter+1, capitalize(bp_flags[counter]) );
	if ( ++col % 2 == 0 )
	    strcat( buf, "\n\r" );
	send_to_char_color( buf, d->character );
    }
...
void oedit_parse( DESCRIPTOR_DATA *d, char *arg )
{
        ...
        switch ( UPPER(arg[0]) )
	{
        ...
        case '9':
            ...
            break;
        case 'A':
            send_to_char("Enter numerical size of object : ", d->character);
        ...
        case 'B':
            ...
            break;
        case 'C':
            send_to_char("Enter mana : ", d->character);
            OLC_MODE(d) = OEDIT_MANA;
            break;
        case 'D':
	    if ( xIS_SET( obj->parts, WEAR_BODY )
	    ||   xIS_SET( obj->parts, WEAR_ABOUT )
	    ||   xIS_SET( obj->parts, WEAR_ARMS )
	    ||   xIS_SET( obj->parts, WEAR_FEET )
	    ||   xIS_SET( obj->parts, WEAR_HANDS )
	    ||   xIS_SET( obj->parts, WEAR_LEGS )
	    ||   xIS_SET( obj->parts, WEAR_WAIST ) )
	    {
	...
	if ( is_number( arg ) )
	{
		...
		xTOGGLE_BIT( obj->parts, number );
		olc_log( d, "%s the wearloc %s",
		    xIS_SET( obj->parts, number ) ? "Added" : "Removed",
		    w_flags[number] );
	    }
	}
	else
    ...
    case OEDIT_SIZE:
        number = atoi(arg);
        obj->size = number;
	olc_log( d, "Changed size to %d", obj->size	);
		if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	...
	case OEDIT_MANA:
        number = atoi(arg);
        obj->mana = number;
	olc_log( d, "Changed mana to %d", obj->mana	);
		if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
				obj->pIndexData->size = obj->mana;
        break;
