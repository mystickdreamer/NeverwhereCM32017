...
void do_instaroom( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->in_room->area != ch->pcdata->area )
   {
...
void reset_room( ROOM_INDEX_DATA * room )
{
   ...
   if( IS_SET( room->room_flags, ROOM_BURNING ) )
   {
      if( room->curr_vegetation > 0 && room->curr_water < 30 )
         room->curr_vegetation -= 25;
      else
         REMOVE_BIT( room->room_flags, ROOM_BURNING );
   }
   else
   {
      if( room->curr_vegetation < room->vegetation )
         room->curr_vegetation++;
   }

   /* Update water in room if it is more than what was set. */
   if( room->curr_water != room->water )
   {
      if( room->first_person )
      {
         if( room->curr_water > room->water )
         {
            act( AT_PLAIN, "The water recedes to its normal level.", room->first_person, NULL, NULL, TO_CHAR );
            act( AT_PLAIN, "The water recedes to its normal level", room->first_person, NULL, NULL, TO_ROOM );
         }
         else
         {
            act( AT_PLAIN, "The water increases to its normal level.", room->first_person, NULL, NULL, TO_CHAR );
            act( AT_PLAIN, "The water increases to its normal level", room->first_person, NULL, NULL, TO_ROOM );
         }
      }
      room->curr_water = room->water;
   }
                  ...
                  switch ( tReset->command )
                  {
                     ...
                     case 'T':
                        ...
                        else
                        {
                           ...
                           pobj = make_trap( tReset->arg2, tReset->arg1, 0, tReset->extra );
