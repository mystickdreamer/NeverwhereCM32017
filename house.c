...
void do_house( CHAR_DATA *ch, char *argument )
{
   ...
   case SUB_ROOM_DESC:
	...
	if ( (!homedata || homedata->apartment) && !IS_SET( ch->pcdata->permissions, PERMIT_ADMIN ))
	{
   ...
   if ( !str_cmp( arg, "set"))
   {
	   ...
	   if ( arg2 == NULL || arg2[0] == '\0' || arg3 == NULL || arg3[0] == '\0' 
	   ...
	   if (!str_cmp( arg3, "addroom"))
	   {
		   ...
		   if (!argument || argument[0] == '\0' || arg3 == NULL || arg3[0] == '\0' || !is_number(arg3) )
		   {
			   send_to_char_color( "&C&wHouse set <character name> addroom <current house vnum> <direction>\n\r", ch);
			   return;
		   }
...
void do_gohome( CHAR_DATA *ch, char *argument )
{
     ...
     switch( ch->position )
     {
         ...
         case POS_SLEEPING:
          ...
          return;
         case POS_SHOVE:
			send_to_char( "&C&wReally? While your shoving someone?\n\r", ch);
         case POS_DRAG:
			send_to_char( "&C&wNot while you're dragging somebody!\n\r", ch);
         case POS_FLYING:
		  do_land( ch, "auto" );
         case POS_MOUNTED:
			do_dismount( ch, "auto" );
...
void do_accessories(CHAR_DATA *ch, char *argument)
{
   ...
   sh_int i = 0;
    ...
    if ( !homedata && !IS_SET( ch->pcdata->permissions, PERMIT_ADMIN ))
    {
	...
	if ( arg == NULL || arg[0] == '\0' || (is_number(arg) && arg2[0] == '\0'))
	{
	...
	if (!str_cmp( arg2, "show"))
	{
		...
		else
		{
			...
			ch_printf_color( ch, "&C&gAverage Hp: &w%d\n\r", 
				( !mob->hitnodice ? (mob->hitnodice * number_range(1, mob->hitsizedice)) :(mob->hitnodice * number_range(1, mob->hitsizedice ) 
				+ mob->hitplus)));
	...
	if (!str_cmp( arg2, "buy"))
	{
		...
		if ( (tmpacc->mob && (mobindex = get_mob_index(tmpacc->vnum)) == NULL)
			|| (!tmpacc->mob && (pObjIndex = get_obj_index( tmpacc->vnum)) == NULL))
		{
...
void do_homebuy(CHAR_DATA *ch, char *argument)
{
   ...
   sh_int i = 0;
	...
	for ( tmphome = first_home; tmphome; tmphome = tmphome->next)
	{
		if ( !str_cmp( tmphome->name, ch->name) && !IS_SET( ch->pcdata->permissions, PERMIT_ADMIN ))
		{
	...
	if ( !str_cmp( arg2, "timeremainder"))
	{
		...
		if ( arg3 == NULL || arg3[0] == '\0' || !is_number( arg3))
		{
...
bool set_house( CHAR_DATA *ch, int vnum, bool apartment)
{
   ...
   if (!ch || !ch->name || vnum <= 0)
	...
	xSET_BIT( key->extra_flags, ITEM_TAKE );
	//xSET_BIT( key->extra_flags, ITEM_HOLD );
...
void homebuy_update(CHAR_DATA *ch, char *argument)
{
   ...
   for ( tmphome = first_homebuy; tmphome; tmphome = tmphomenext)
   {
	   ...
	   if ( !remove_house( seller) && !IS_SET( ch->pcdata->permissions, PERMIT_ADMIN )(seller))
	   {
	   ...
	   if ( !remove_house( seller) && !IS_SET( ch->pcdata->permissions, PERMIT_ADMIN ))
	   {
