...
void do_build_walk( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET( ch->pcdata->permissions, PERMIT_BUILD ) && IS_SET( ch->pcdata->flags, PCFLAG_BUILDWALK ) )
    {
...
void explore_room( CHAR_DATA * ch, ROOM_INDEX_DATA * room )
{
   ...
   while( xarea )
   {
      ...
      {
         if( xIS_SET( xarea->rooms, ch->in_room->vnum - ch->in_room->area->low_r_vnum ) )
            return;
         xSET_BIT( xarea->rooms, ch->in_room->vnum - ch->in_room->area->low_r_vnum );
         ch->exp += 100000;
         return;
      }
      xarea = xarea->next;
...
bool will_fall( CHAR_DATA * ch, int fall )
{
   ...
   if( IS_AFFECTED( ch, AFF_FLYING ) &&
       ...
       ch->curr_talent[TAL_MOTION] < 60 && ( !can_use_bodypart( ch, BP_RWING ) || !can_use_bodypart( ch, BP_LWING ) ) )
   {
   ...
   if( IS_SET( ch->in_room->room_flags, ROOM_NOFLOOR ) && CAN_GO( ch, DIR_DOWN ) )
   {
      ...
      if( IS_UNDERWATER( ch ) )
      {
         if( ch->in_obj && IS_SET( ch->in_obj->value[2], VEHICLE_UNDERWATER ) )
            return FALSE;
      ...
      {
         if( ch->in_obj || ( !IS_AFFECTED( ch, AFF_FLYING ) && ( !ch->mount || !IS_AFFECTED( ch->mount, AFF_FLYING ) ) ) )
         {
            if( fall > 80 )
            {
...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
   ...
   if( IS_NPC( ch )
       && ( IS_SET( pexit->to_room->room_flags, ROOM_NO_MOB ) || IS_SET( pexit->to_room->runes, RUNE_DISTRACT ) ) )
   {
   ...
   if( IS_SET( pexit->to_room->room_flags, ROOM_NO_DREAM )
       && IS_AFFECTED( ch, AFF_DREAMWORLD ) && ch->curr_talent[TAL_DREAM] < 100 )
   {
   ...
   if( room_is_private( to_room ) && ( IS_NPC( ch ) || !IS_SET( ch->pcdata->permissions, PERMIT_BUILD ) ) )
   {
   ...
   if( !fall )
   {
      ...
      if( ch->in_obj )
      {
         ...
         if( !IS_SET( ch->in_obj->value[2], VEHICLE_SPACE ) && to_room->sector_type == SECT_DUNNO )
         {
            send_to_char( "Your vehicle cannot clear the atmosphere.\n\r", ch );
            return rNONE;
         }
         if( ( to_room->sector_type == SECT_UNDERWATER
               || to_room->sector_type == SECT_OCEANFLOOR ) && !IS_SET( ch->in_obj->value[2], VEHICLE_UNDERWATER ) )
         {
         ...
         if( !IS_SET( ch->in_obj->value[2], VEHICLE_AIR ) )
         {
            ...
            if( ( to_room->sector_type == SECT_MOUNTAIN
                  ...
                  || to_room->sector_type == SECT_FOREST ) && !IS_SET( ch->in_obj->value[2], VEHICLE_CLIMB ) )
            {
               send_to_char( "Your vehicle cannot handle that rough terrain.\n\r", ch );
               return rNONE;
            }
            else if( ( to_room->sector_type == SECT_DESERT
                       || to_room->sector_type == SECT_BEACH ) && !IS_SET( ch->in_obj->value[2], VEHICLE_SAND ) )
            {
               send_to_char( "Your vehicle would founder in the sand.\n\r", ch );
               return rNONE;
            }
            else if( ( to_room->sector_type == SECT_WATER_SWIM
                       || to_room->sector_type == SECT_WATER_NOSWIM ) && !IS_SET( ch->in_obj->value[2], VEHICLE_SURFACE ) )
            {
               send_to_char( "Your vehicle cannot float on water.\n\r", ch );
               return rNONE;
            }
            else if( ( to_room->sector_type == SECT_ICE
                       || IS_SET( to_room->room_flags, ROOM_SLIPPERY ) ) && !IS_SET( ch->in_obj->value[2], VEHICLE_ICE ) )
            {
               send_to_char( "Your vehicle would slip and spin out of control on the ice.\n\r", ch );
               return rNONE;
            }
            else if( ( to_room->sector_type == SECT_CITY
                       || to_room->sector_type == SECT_FIELD ) && !IS_SET( ch->in_obj->value[2], VEHICLE_LAND ) )
            {
               send_to_char( "Your vehicle is not built to travel on land.\n\r", ch );
               return rNONE;
            }
         }
         if( to_room->sector_type == SECT_UNDERGROUND && !IS_SET( ch->in_obj->value[2], VEHICLE_UNDERGROUND ) )
         {
      ...
      if( ( in_room->sector_type == SECT_AIR
            || to_room->sector_type == SECT_AIR || IS_SET( pexit->exit_info, EX_FLY ) ) && pexit->vdir == 4 )
      {
         ...
         else if( ch->mount && !IS_AFFECTED( ch, AFF_MOUNTED ) && !IS_AFFECTED( ch->mount, AFF_FLYING ) )
         {
   ...
   if( !ch->in_obj && ( ch->in_room->sector_type == SECT_UNDERWATER || ch->in_room->sector_type == SECT_OCEANFLOOR ) )
      if( ( ch->position <= POS_STANDING ) && ( !fall ) )
      {
         ...
         {
            act( AT_BLUE, "You splash about but are unable to get anywhere.", ch, NULL, NULL, TO_CHAR );
            act( AT_BLUE, "$n splashes about ineffectively in the water.", ch, NULL, NULL, TO_ROOM );
            learn_from_failure( ch, gsn_swim );
            STRFREE( ch->last_taken );
...
/* slippery floor, chance of falling -keo */
   if( IS_SET( ch->in_room->room_flags, ROOM_SLIPPERY ) && !IS_AFFECTED( ch, AFF_FLYING ) && !fall && !ch->in_obj )
   {
      if( number_range( 1, 200 ) > get_curr_dex( ch ) + ch->curr_talent[TAL_FROST] )
      {
...
/* sticky floor, chance of sticking -keo */
   if( IS_SET( ch->in_room->room_flags, ROOM_STICKY ) && !IS_AFFECTED( ch, AFF_FLYING ) && !fall )
   {
...
/* check for guards -keo */
   ...
   {
      if( gch->position > POS_KNEELING && IS_SET( ch->mood, MOOD_GUARD ) && ( gch != ch ) )
      {
         if( IS_SAME_PLANE( ch, gch ) )
         {
            if( IS_AFFECTED( ch, AFF_FLYING ) && !IS_AFFECTED( gch, AFF_FLYING ) )
            {
            ...
            else
            {
               ...
               {
                  act( AT_PLAIN, "$N tries to block you but you bowl $M over!", ch, NULL, gch, TO_CHAR );
                  act( AT_PLAIN, "You try to block $n but $e bowls you over!", ch, NULL, gch, TO_VICT );
...
/* Everything went through, move them! */
   ...
   if( !IS_AFFECTED( ch, AFF_SNEAK )
       || ( !IS_NPC( ch ) && number_range( 1, 20 ) > number_range( 1, ch->pcdata->noncombat[SK_STEALTH] ) ) )
   {
      if( fall )
         if( IS_UNDERWATER( ch ) )
            txt = "sinks";
   ...
   if( !fall )
   {
      ...
      for( fch = from_room->first_person; fch && ( count < chars ); fch = nextinroom )
      {
         ...
         if( fch != ch  /* loop room bug fix here by Thoric */
             && fch->master == ch && fch->position >= POS_SQUATTING && can_see( fch, ch ) )
         {
   ...
   if( IS_AFFECTED( ch, AFF_VOID ) && number_range( 1, 25 ) > ch->curr_talent[TAL_VOID] )
   {
      travel_teleport( ch );
   ...
   if( !will_fall( ch, fall ) && fall > 0 )
   {
      ...
      else if( !IS_AFFECTED( ch, AFF_FLOATING ) || ( ch->mount && !IS_AFFECTED( ch->mount, AFF_FLOATING ) ) )
      {
         set_char_color( AT_HURT, ch );
         send_to_char( "OUCH! You hit the ground!\n\r", ch );
         lose_hp( ch, 30 * fall );
...
OBJ_DATA *has_key( CHAR_DATA * ch, int key )
{
   ...
   {
      if( obj->item_type == ITEM_KEY && obj->pIndexData->vnum == key || obj->item_type == ITEM_KEY && obj->value[0] == key )
         return obj;
...
void do_bashdoor( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch ) && LEARNED( ch, gsn_bashdoor ) < 1 )
   {
   ...
   if( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
   {
      ...
      if( !IS_SET( pexit->exit_info, EX_BASHPROOF )
          && ch->move >= 15 && number_percent(  ) < ( chance + get_curr_str( ch ) ) )
      {
         ...
         if( can_use_bodypart( ch, BP_RHOOF ) || can_use_bodypart( ch, BP_LHOOF ) )
         {
      ...
      else
      {
         if( can_use_bodypart( ch, BP_RHOOF ) || can_use_bodypart( ch, BP_LHOOF ) )
         {
            ...
            act( AT_SKILL, "WHAAAAM!!! $n kicks the $d, but it holds firm.", ch, NULL, keyword, TO_ROOM );
            STRFREE( ch->last_taken );
             ...
             && ( pexit_rev = pexit->rexit ) != NULL && pexit_rev->to_room == ch->in_room )
         {
            ...
            for( rch = to_room->first_person; rch; rch = rch->next_in_room )
            {
               act( AT_SKILL, message, rch, NULL, pexit_rev->keyword, TO_CHAR );
            }
         }
      }
   }
...
void do_stand( CHAR_DATA * ch, char *argument )
{
   ...
   {
      case POS_SLEEPING:
         if( IS_AFFECTED( ch, AFF_SLEEP ) || IS_AFFECTED( ch, AFF_DREAMWORLD ) )
         {
...
void do_enter( CHAR_DATA * ch, char *argument )
{
    ...
    */
   if( !obj || ( obj->item_type != ITEM_VEHICLE && obj->item_type != ITEM_FOUNTAIN ) )
   {
   ...
   ch->in_obj = obj;
   if( obj->item_type == ITEM_FOUNTAIN && IS_AFFECTED( ch, AFF_FLAMING ) )
   {
...
void do_leave( CHAR_DATA * ch, char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      {
         obj = ch->in_obj;
         act( AT_ACTION, "$n gets out of $p.", ch, ch->in_obj, NULL, TO_ROOM );
         act( AT_ACTION, "You get out of $p.", ch, ch->in_obj, NULL, TO_CHAR );
         ch->in_obj = NULL;
         return;
      }
      for( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
...
void do_shove( CHAR_DATA * ch, char *argument )
{
   ...
   chance +=
      ( ( get_curr_str( ch ) + ch->weight ) - ( ( get_curr_str( victim ) + get_curr_dex( victim ) ) / 2 ) + victim->weight );
...
{
   if( !can_use_bodypart( ch, BP_RWING ) && !can_use_bodypart( ch, BP_LWING ) )
   {
   ...
   act( AT_PLAIN, "You spread your wings and take to the sky.", ch, NULL, NULL, TO_CHAR );
   act( AT_PLAIN, "$n spreads $s wings and takes to the sky.", ch, NULL, NULL, TO_ROOM );
   if( IS_NPC( ch ) )
