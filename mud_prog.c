...
int mprog_do_ifcheck
args( ( char *ifcheck, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, CHAR_DATA * rndm ) );
void mprog_translate
args( ( char ch, char *t, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, CHAR_DATA * rndm ) );
void mprog_driver
args( ( char *com_list, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, bool single_step ) );
...
/* Changed below from 1 to 0 for == 0 Shaddai */ 
         ...
         
      {
         ...
         for( ch = first_char; ch; ch = ch->next )
            if( !IS_NPC( chkchar ) && !IS_NPC( ch ) 
                 &&ch->desc  &&chkchar->desc  &&ch->desc->host == chkchar->desc->host )
               lhsvl++;
         rhsvl = atoi( rval );
         if( rhsvl < 0 )
            rhsvl = 0;
      ...
      
   {
      ...
      for( d = first_descriptor; d; d = d->next )
         if( d->connected == CON_PLAYING  &&d->character  &&nifty_is_name( d->character->name, cvar ) )
            return TRUE;
      return FALSE;
   }
      ...
      
   {
      ...
      for( ch = mob->in_room->first_person; ch; ch = ch->next_in_room )
         if( ( !IS_NPC( ch ) )  &&nifty_is_name( ch->name, cvar ) )
            return TRUE;
      return FALSE;
   }
      ...
      
   {
      ...
      room = get_room_index( rvnum ? rvnum : mob->in_room->vnum );
      for( tch = room ? room->first_person : NULL; tch; tch = tch->next_in_room )
         if( ( ( !IS_NPC( tch ) )  )  ||IS_NPC( tch ) /* mortal or mob */  
             )
            count++;
      return mprog_veval( count, opr, atoi( rval ), mob );
   }
...
void mprog_greet_trigger( CHAR_DATA * ch ) 
{
      ...
      
   {
      ...
      if( !IS_NPC( vmob )  ||!can_see( vmob, ch )  ||!IS_AWAKE( vmob ) )
         continue;
