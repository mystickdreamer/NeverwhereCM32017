...
void web_imc_list( WEB_DESCRIPTOR * wdesc )
{
   REMOTEINFO *rin;
   char *start, *onpath;
   char buf[MAX_STRING_LENGTH], urldump[MAX_INPUT_LENGTH], netname[MAX_INPUT_LENGTH], routerpath[MAX_INPUT_LENGTH];
   int end, count = 1;
   ...
   else
   {
      ...
      snprintf( buf, MAX_STRING_LENGTH,
                ...
                urldump, this_imcmud->localname, this_imcmud->versionid, this_imcmud->network, this_imcmud->routername );
   }
   send_buf( wdesc->fd, buf );
   ...
   for( rin = first_rinfo; rin; rin = rin->next, count++ )
   {
      ...
      if( rin->path && rin->path[0] != '\0' )
      {
         if( ( start = strchr( rin->path, '!' ) ) != NULL )
         {
            ...
            imcstrlcpy( routerpath, rin->path, MAX_INPUT_LENGTH );
      }
      ...
      if( !rin->url || !str_cmp( rin->url, "??" ) || !str_cmp( rin->url, "Unknown" ) || !strstr( rin->url, "http://" ) )
      {
         snprintf( buf, MAX_STRING_LENGTH,
                   ...
                   rin->expired ? "red" : "#008080", rin->name, rin->version, netname, routerpath );
      }
      else
      {
         ...
         snprintf( buf, MAX_STRING_LENGTH,
                   ...
                   rin->name, rin->version, netname, routerpath );
      }
      send_buf( wdesc->fd, buf );
   }
   imcstrlcpy( buf,
