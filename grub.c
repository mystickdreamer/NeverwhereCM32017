   ...
   for( obj = first_object; obj; obj = obj->next )
   {
      ...
      if( outer_obj->carried_by )
      {
         ...
         sprintf( buf + strlen( buf ), "%5d %-18s ",
                  ( IS_NPC( outer_obj->carried_by ) ? outer_obj->carried_by->pIndexData->vnum : 0 ), field );
         if( outer_obj != obj )
         {
            sprintf( field, "%-18s", obj->in_obj->name );
            gtrunc( field, 18 );
            sprintf( buf + strlen( buf ), "%5d %-18s ", obj->in_obj->pIndexData->vnum, field );
         }
      ...
      else if( outer_obj->in_room )
      {
         ...
         gtrunc( field, 18 );
         sprintf( buf + strlen( buf ), "%5d %-18s ", outer_obj->in_room->vnum, field );
         if( outer_obj != obj )
         {
            sprintf( field, "%-18s", obj->in_obj->name );
            gtrunc( field, 18 );
...
void do_opfind( CHAR_DATA * ch, char *argument )   /* Gorog */
{
   ...
   if( str_cmp( arg2, "mud" ) )
   {
      ...
      if( arg3[0] == '\0' || argument[0] == '\0' || !is_number( arg2 ) || !is_number( arg3 ) )
      {
         opfind_help( ch );
         return;
      }
...
void display_operand_table( CHAR_DATA * ch, int op_num )
{
   ...
   for( cou = 0; cou < op_num; cou++ )
      if( go_op[cou].num )
         pager_printf( ch,
                       "%2d %-7s %2s %10ld\n\r", cou + 1, go_fd[go_op[cou].field].nam, opn[go_op[cou].op], go_op[cou].nval );
      else
...
void gr_init( void )
{
   ...
   gr_fd[6].num = TRUE;
   strcpy( gr_fd[7].nam, "site" );
   gr_fd[7].num = FALSE;
   strcpy( gr_fd[8].nam, "last" );
   gr_fd[8].num = TRUE;
   strcpy( gr_fd[9].nam, "pkill" );
   gr_fd[9].num = FALSE;
}
      ...
      if( res )   /* record is a match         */
      {
         ...
         if( !title_sw && dis_num > 0 )   /* print title if applicable */
         {
            ch_printf( ch,
                       ...
                       "Name", "Lv", "S", "R", "C", "Room", "Gold", "Site", "Last", "Pk" );
            title_sw = TRUE;
         }
         ...
         for( pObj = obj_index_hash[hash]; pObj; pObj = pObj->next )
            if( pObj->layers > 0 )
            {
               if( ++cou <= display_limit )
                  pager_printf( ch, "%4d %5d %5d   %s\n\r", cou, pObj->vnum, pObj->layers, pObj->short_descr );
            }
}
...
#define   DIAG_MAX_SIZE  1000
   ...
   if( !str_cmp( arg1, "time" ) )
   {
      ...
      pager_printf( ch, "mon=%d day=%d hh=%d mm=%d\n\r", t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min );
      return;
   }
...
*/
   ...
   if( !str_cmp( arg1, "mrc" ) )
   {
      ...
      if( !*arg2 || !*arg3 || !*arg4 || !*arg5 || !*arg6
          || !isdigit( (int)*arg2 ) || !isdigit( (int)*arg3 ) || !isdigit( (int)*arg4 ) || !isdigit( (int)*arg5 ) || !isdigit( (int)*arg6 ) )
      {
