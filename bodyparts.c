...
void check_bodyparts( CHAR_DATA * ch )
{
   ...
   for( i = 0; i < MAX_PARTS; i++ )
   {
      ...
      add_part( ch, BP_GROIN, BP_WAIST );
   
   else
      remove_part( ch, BP_GROIN );
...
bool sever_part( CHAR_DATA * ch, PART_DATA * part )
{
   ...
   if( part->flags == PART_SEVERED )
      ...
      if( part->loc == BP_CHEST 
          ||part->loc == BP_WAIST 
          ||part->loc == BP_GROIN  ||part->loc == BP_STOMACH  ||part->loc == BP_BACK  ||part->loc == BP_FACE )
   {
      return FALSE;
   }
...
void break_part( CHAR_DATA * ch, PART_DATA * part )
{
   if( part->flags != PART_WELL )
      ...
      if( part->loc == BP_STOMACH 
          ||part->loc == BP_REAR 
          ||part->loc == BP_LEAR  ||part->loc == BP_GROIN  ||part->loc == BP_REYE  ||part->loc == BP_LEYE )
   {
      return;
   }
